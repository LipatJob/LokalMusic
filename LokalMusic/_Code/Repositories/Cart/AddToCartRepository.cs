...
namespace LokalMusic._Code.Repositories.Cart
{
    public class AddToCartRepository
    {
        ...
        // Validations before adding to cart
        ...
        public static bool IsProductBought(int productId, int userId)
        {
            string query = @"
                            SELECT 
                                ProductOrderId
                            FROM ProductOrder
                                INNER JOIN OrderInfo ON ProductOrder.OrderId = OrderInfo.OrderId
                            WHERE 
        ...
        public static bool IsUsersProductAlbum(int productId, int userId)
        {
            string query = @"
                            ...
                            WHERE 
                                Album.UserId = @UserArtistId AND Product.ProductId = @ProductId";
        ...
        public static int GetAlbumIdOfTrack(int trackId)
        {
            string query = @"
                            SELECT 
                                AlbumId
                            FROM Product
                                INNER JOIN Track ON ProductId = TrackId
                            WHERE 
                                ProductId = @TrackId";
