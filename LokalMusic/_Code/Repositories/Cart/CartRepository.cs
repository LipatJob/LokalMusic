...
namespace LokalMusic._Code.Repositories.Cart
{
    public class CartRepository
    {
        ...
        // Queries for displaying cart: albums in cart
        public List<CartAlbum> GetAlbums(int customerId)
        {
            ...
            string query = $@"
                            SELECT 
                                ...
                                ArtistName, AlbumProduct.Price, 
                                FileName as AlbumCoverAddress
                            FROM UserCart
                                INNER JOIN Product as AlbumProduct ON UserCart.ProductId = AlbumProduct.ProductId
                                INNER JOIN Album ON AlbumProduct.ProductId = Album.AlbumId
        ...
        // Queries for displaying cart: tracks in cart. displaying tracks requires the artist
        public List<CartArtist> GetArtistFromCart(int customerId)
        {
            ...
            string query = $@"
                            SELECT 
                                DISTINCT(Album.UserId) as ArtistId, 
                                ArtistName
                            FROM UserCart
                                INNER JOIN Product AS TrackProduct ON UserCart.ProductId = TrackProduct.ProductId
        ...
        public List<CartTrack> GetTracks(int customerId, int albumArtistId)
        {
            ...
            string query = $@"
                            SELECT 
                                TrackId, AlbumProduct.ProductId as AlbumId, Album.UserId as ArtistId,
                                TrackProduct.ProductName as TrackName, 
                                AlbumProduct.ProductName as AlbumName,
                                TrackProduct.Price, 
                                Track.TrackDuration as AudioLength, 
        ...
        // Queries to create the actual order
        public int CreateOrderInfo(int customerId, Decimal amountPaid, string paymentProvider)
        {
            ...
            return (int) DbHelper.ExecuteScalar(query, ("CustomerId", customerId), ("OrderDate", DateTime.Now), ("AmountPaid", amountPaid), ("PaymentProvider", paymentProvider));
        }
        ...
        // Remove selected displayed products from the cart
        ...
        public List<int> GetAlbumTrackIds(int albumId)
        {
            ...
            string query = @"
                            ...
                            FROM Product as AlbumProduct
                                INNER JOIN Track ON AlbumProduct.ProductId = Track.AlbumId
                            WHERE 
                                AlbumProduct.ProductId = @AlbumId";
        ...
        // Queries to remove the ordered product(s) from the cart
        public void RemoveTrackFromCart(int customerId, int trackId)
        {
            ...
            DbHelper.ExecuteNonQuery(query, ("CustomerId", customerId), ("TrackId", trackId));
        }
        public void RemoveAlbumFromCart(int customerId, int albumId)
        {
            ...
            DbHelper.ExecuteNonQuery(query, ("CustomerId", customerId), ("AlbumId", albumId));
        }
    }
}