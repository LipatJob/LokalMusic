...
namespace LokalMusic._Code.Repositories.Fan
{
    public class PlaylistRepository
    {
        ...
        // Queries intended to retrieved purchased album
        public List<PlaylistAlbum> GetPlaylistAlbum(int customerId)
        {
            ...
            string query = $@"
                            ...
                            FROM OrderInfo 
                                ...
                                INNER JOIN ArtistInfo ON ArtistInfo.UserId = UserInfo.UserId
                            WHERE 
            ...
            if (valid)
            {
                for (int i = 0; i < values.Rows.Count; i++)
                {
                    ...
                    album.tracks = this.GetPlaylistTrack(album.AlbumId);

                    // only add the entry if there is/are track(s) found
                    if (album.tracks != null) albums.Add(album);
                }
            }
        ...
        public List<PlaylistTrack> GetPlaylistTrack(int albumId)
        {
            ...
            string query = $@"
                            SELECT 
                                Track.TrackId, 
                                ProductName as TrackName, 
                                FileName as AudioAddress
                            FROM Product
                                INNER JOIN Track ON Product.ProductId = Track.TrackId
                                INNER JOIN FileInfo ON Track.TrackFileID = FileInfo.FileId
                            WHERE 
        ...
        // Queries to retrieved purchased tracks' album
        ...
        public List<PlaylistTrack> GetIncompleteTracks(int customerId, int albumId)
        {
            ...
            string query = $@"
                            SELECT 
                                ...
                                FileInfo.FileName as AudioAddress
                            FROM OrderInfo 
                                INNER JOIN ProductOrder ON OrderInfo.OrderId = ProductOrder.OrderId
                                INNER JOIN Product ON ProductOrder.ProductId = Product.ProductId
                                ...
                                INNER JOIN FileInfo ON Track.TrackFileID = FileInfo.FileId
                            WHERE 
                                OrderInfo.CustomerId = @CustomerId AND Track.AlbumId = @AlbumId
                                AND Product.ProductStatusId = (SELECT ProductStatus.ProductStatusId FROM ProductStatus WHERE ProductStatus.StatusName = '{STATUS_PRODUCT_VISIBLE}')";
